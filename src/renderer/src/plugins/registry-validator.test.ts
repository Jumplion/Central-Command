...
describe('getWidgetRegistrationError', () => {
  ...
  it('returns undefined for a valid widget', () => {
    const widget = {
      default: {
        manifest: { id: 'foo', name: 'Foo', version: '0.1.0', defaultSize: { w: 2, h: 2 } },
        Component: () => null
      }
    };
    const usedIds = new Set<string>();
    expect(getWidgetRegistrationError(widget, usedIds, currentPlatform)).toBeUndefined();
  });
  ...
  it('rejects invalid widget ids', () => {
    const widget = {
      default: {
        manifest: { id: 'BadId', name: 'Bad', version: '0.1.0', defaultSize: { w: 1, h: 1 } },
        Component: () => null
      }
    };
    expect(getWidgetRegistrationError(widget, new Set(), currentPlatform)).toBe('invalid widget id "BadId"');
  });

  it('skips widgets for unsupported platforms without warning', () => {
    const widget = {
      default: {
        manifest: { id: 'bar', name: 'Bar', version: '0.1.0', defaultSize: { w: 1, h: 1 }, platforms: ['mobile'] },
        Component: () => null
      }
    };
    expect(getWidgetRegistrationError(widget, new Set(), currentPlatform)).toBe('unsupported platform');
  });
