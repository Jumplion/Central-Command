...
type DashboardState = {
  ...
  updateLayout: (layout: Array<{ instanceId: string; x: number; y: number; w: number; h: number }>) => void;
};
...
let widgets: Widget[] = [
  ...
  },
  {
    manifest: {
      ...
      defaultSize: { w: 3, h: 2 },
      settings: [
        { kind: 'string', key: 'note', label: 'Note', placeholder: 'Enter note' },
      ],
    },
...
vi.mock('@renderer/state/dashboard', () => ({
  useDashboard: (selector: (state: DashboardState) => unknown) => selector(dashboardState),
}));
...
vi.mock('@renderer/plugins/api', () => ({
  createWidgetApi: vi.fn((widgetId: string, instanceId: string) => ({
    ...
    sql: { run: vi.fn(), all: vi.fn(), get: vi.fn(), exec: vi.fn(), runBatch: vi.fn() },
...
const dispatchValueEvent = async (element: HTMLElement, value: string | boolean) => {
  await act(async () => {
...
beforeEach(() => {
  ...
  (window as any).cc = {
    driveSync: {
      getStatus: vi.fn(async () => ({ enabled: false, state: 'disabled', lastSyncedAt: null, lastError: null })),
...
describe('AddWidgetDialog', () => {
  it('renders widget list, filters results, and adds a widget', async () => {
    ...
    const addButton = container.querySelector<HTMLButtonElement>('button.primary');
...
describe('AppSettings', () => {
  ...
  it('shows connected status and allows disconnect', async () => {
    ...
    const disconnect = Array.from(container.querySelectorAll('button')).find((button) =>
...
describe('WidgetHost', () => {
  it('renders missing widget UI and removes the instance', async () => {
    const container = createContainer();
    const root = createRoot(container);
    const instance = { instanceId: 'missing-1', widgetId: 'missing-widget', title: undefined, settings: {}, layout: { x: 0, y: 0, w: 2, h: 2 } };
  ...
  it('renders widget body and toggles settings panel when manifest has settings', async () => {
    ...
    const container = createContainer();
    const root = createRoot(container);
    const instance = { instanceId: 'inst-c', widgetId: 'widget-c', title: undefined, settings: { active: true }, layout: { x: 0, y: 0, w: 2, h: 2 } };
  ...
  it('displays widget error UI when child component throws', async () => {
    ...
    const container = createContainer();
    const root = createRoot(container);
    const instance = { instanceId: 'inst-crash', widgetId: 'widget-crash', title: undefined, settings: {}, layout: { x: 0, y: 0, w: 2, h: 2 } };
  ...
  it('renders each field type and saves updated settings and title override', async () => {
    const widget = {
      manifest: {
        ...
        icon: 'S',
        settings: [
          { kind: 'string', key: 'text', label: 'Text', placeholder: 'Enter text' },
          { kind: 'number', key: 'count', label: 'Count', min: 0, max: 10, step: 1 },
    ...
    await act(async () => {
      root.render(<WidgetSettingsPanel widget={widget as any} instance={instance as any} onClose={onClose} />);
    });

    const titleInput = container.querySelector<HTMLInputElement>('input[placeholder="Settings Widget"]');
    const textInput = container.querySelector<HTMLInputElement>('input[placeholder="Enter text"]');
    ...
    expect(dashboardState.updateSettings).toHaveBeenCalledWith('inst-settings', {
      text: 'world',
      count: 5,
      enabled: true,
      mode: 'two',
    });
    expect(dashboardState.setTitle).toHaveBeenCalledWith('inst-settings', 'New title');
    expect(onClose).toHaveBeenCalledOnce();
    cleanupContainer(container);
  });
});
...
describe('Sidebar', () => {
  it('renders dashboard tabs and opens add widget and settings dialogs', async () => {
    ...
    const addWidgetButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === '+ Add widget');
  ...
  it('calls setActiveDashboard and addDashboard from dashboard buttons', async () => {
    ...
    const workButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Work');
    await act(async () => {
      workButton?.click();
    });
    ...
    const newDashboardButton = Array.from(container.querySelectorAll('button')).find((button) => button.title === 'New dashboard');
    await act(async () => {
  ...
  it('confirms and removes the active dashboard', async () => {
    ...
    const deleteButtons = Array.from(container.querySelectorAll('button')).filter((button) => button.ariaLabel === 'Delete dashboard');
