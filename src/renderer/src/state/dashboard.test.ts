...
function makeWidget(id = 'test-widget') {
  return {
    manifest: { id, name: 'Test Widget', version: '1.0.0', defaultSize: { w: 3, h: 4 } },
    Component: () => null,
  };
}
...
beforeEach(() => {
  ...
  useDashboard.setState({ state: structuredClone(DEFAULT_STATE), loaded: false });
});
...
describe('renameDashboard', () => {
  it('updates the name of the specified dashboard', () => {
    ...
    expect(getState().dashboards.find((d) => d.id === 'default')?.name).toBe('Renamed');
  });
});
...
describe('addInstance', () => {
  ...
  it('adds a widget instance with correct layout and settings', () => {
    ...
    const instances = getState().dashboards.find((d) => d.id === getState().activeDashboardId)!.instances;
...
describe('updateSettings', () => {
  it('merges new settings onto the specified instance', () => {
    ...
    const instance = getState().dashboards[0].instances.find((i) => i.instanceId === instanceId)!;
