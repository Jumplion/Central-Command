...
describe("searchArbeitnow", () => {
  ...
  it("throws a status error when the request fails", async () => {
    ...
    await expect(searchArbeitnow(fetch, "frontend", false)).rejects.toThrow(
      "Arbeitnow error 403",
    );
  });
...
describe("fetchFeed", () => {
  it("fetches and stamps lever feeds", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://api.lever.co/v0/postings/leverco?mode=xml",
    );
  ...
  it("fetches greenhouse feeds", async () => {
    ...
    const result = await fetchFeed(fetch, feed, "");
    expect(fetch).toHaveBeenCalledWith(
      "https://api.greenhouse.io/v1/boards/greenhouseco/jobs",
    );
    expect(result[0]).toMatchObject({
      ext_id: "gh-1",
      title: "Frontend Engineer",
  ...
  it("throws for unknown feed types", async () => {
    ...
    await expect(fetchFeed(fetch, feed, "key")).rejects.toThrow(
      "Unknown feed type: unknown",
    );
  });
});
