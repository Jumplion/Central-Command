...
export function BoardsTab({ api, feeds, feedJobs, savedIds, onFeedsChange, onSaved }: Props) {
  ...
  const visibleFeeds = useMemo(() => feeds.filter((f) => {
    ...
    if (companyTypeFilter !== 'all' && (f.company_type ?? 'other') !== companyTypeFilter) return false;
  ...
  const grouped = useMemo(() =>
    COMPANY_TYPE_ORDER.reduce<Record<CompanyType, CompanyFeed[]>>((acc, t) => {
      acc[t] = filteredFeeds.filter((f) => (f.company_type ?? 'other') === t);
      return acc;
      ...
      <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          {(['all', 'lever', 'greenhouse', 'rss', 'search'] as const).map((t) => {
            ...
            return (
              <button key={t} onClick={() => setFeedTypeFilter(t)} style={{
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          {(['all', ...COMPANY_TYPE_ORDER] as const).map((t) => {
            ...
            const color  = t === 'all' ? 'var(--accent)' : COMPANY_TYPE_COLORS[t];
            const count  = t === 'all' ? feeds.length : feeds.filter((f) => (f.company_type ?? 'other') === t).length;
            if (count === 0 && t !== 'all') return null;
            return (
              <button key={t} onClick={() => setCompanyTypeFilter(t)} style={{
                fontSize: 11, padding: '2px 8px', borderRadius: 4, cursor: 'pointer', whiteSpace: 'nowrap',
                background: active ? `${color}22` : 'transparent',
                border:     active ? `1px solid ${color}55` : '1px solid transparent',
                color:      active ? color : 'var(--text-dim)',
              }}>
                {t === 'all' ? 'All Industries' : COMPANY_TYPE_LABELS[t]} ({count})
              </button>
            );
          })}
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          {jobQ && totalMatchingJobs !== null && (
            <span style={{ fontSize: 11, color: 'var(--text-dim)', whiteSpace: 'nowrap' }}>
              {totalMatchingJobs} job{totalMatchingJobs !== 1 ? 's' : ''} in {filteredFeeds.filter((f) => filteredFeedJobs[f.id].length > 0).length} compan{filteredFeeds.filter((f) => filteredFeedJobs[f.id].length > 0).length !== 1 ? 'ies' : 'y'}
            </span>
          )}
        </div>
              ...
              <div key={type} style={{ marginBottom: 4 }}>
                ...
                >
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-dim)', flex: 1 }}>
                    ...
                    {jobQ && ` · ${groupFeeds.reduce((n, f) => n + filteredFeedJobs[f.id].length, 0)} matching job${groupFeeds.reduce((n, f) => n + filteredFeedJobs[f.id].length, 0) !== 1 ? 's' : ''}`}
                  </span>
