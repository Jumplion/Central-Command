...
describe('STATUSES', () => {
  it('contains the expected statuses', () => {
    expect(STATUSES).toEqual(['Interested', 'Applied', 'Phone', 'Onsite', 'Offer', 'Rejected']);
  });
...
describe('COMPANY_TYPE_ORDER / COMPANY_TYPE_LABELS / COMPANY_TYPE_COLORS', () => {
  it('COMPANY_TYPE_ORDER length matches COMPANY_TYPE_LABELS key count', () => {
    expect(COMPANY_TYPE_ORDER).toHaveLength(Object.keys(COMPANY_TYPE_LABELS).length);
  });

  it('COMPANY_TYPE_ORDER length matches COMPANY_TYPE_COLORS key count', () => {
    expect(COMPANY_TYPE_ORDER).toHaveLength(Object.keys(COMPANY_TYPE_COLORS).length);
  });
