...
export function JobCard({
  ...
  return (
    <div
      ...
      onMouseEnter={(e) =>
        (e.currentTarget.style.backgroundColor = "rgba(110, 168, 255, 0.07)")
      }
        ...
        <div style={{ minWidth: 0 }}>
          ...
          >
            ...
            {job.isRemote && (
              <>
                {" "}
                · <span style={{ color: "#34d399" }}>Remote</span>
              </>
            )}
          ...
          >
            ...
            {salary && (
              <span style={{ fontSize: 11, color: "#34d399" }}>{salary}</span>
            )}
            {job.datePosted && (
...
const ADD_FEED_META: Record<FeedType, { placeholder: string; hint: string }> = {
  lever: {
    placeholder: "e.g. netflix  (from jobs.lever.co/netflix)",
    hint: "Find it in the jobs.lever.co/YOUR-HANDLE URL",
  },
  greenhouse: {
    placeholder: "e.g. squarespace  (from boards.greenhouse.io/squarespace)",
    hint: "Find it in the boards.greenhouse.io/YOUR-TOKEN URL",
  },
  rss: {
    placeholder: "https://example.com/jobs/feed.xml",
    hint: "Any public RSS or Atom XML feed",
  },
  search: {
    placeholder: "e.g. Toyota Motor  (aggregates from LinkedIn, Indeed, etc.)",
    hint: "Requires a RapidAPI key in widget settings",
  },
};
...
export function AddFeedForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onSave(name.trim(), url.trim(), feedType, companyType);
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      ...
      >
        ...
        {lastFetch && !loading && (
          <span
            ...
            title={new Date(lastFetch).toLocaleString()}
          >
      ...
      {expanded && (
        <div style={{ padding: "6px 10px 10px" }}>
          {error && (
            <div style={{ fontSize: 11, color: "#ff6e6e", marginBottom: 6 }}>
              {error}
            </div>
          )}
