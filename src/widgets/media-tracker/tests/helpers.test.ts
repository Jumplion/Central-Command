...
describe("relationLabel", () => {
  it("returns Sequel for sequel", () =>
    expect(relationLabel("sequel")).toBe("Sequel"));
  it("returns Prequel for prequel", () =>
    expect(relationLabel("prequel")).toBe("Prequel"));
  it("returns Spinoff for spinoff", () =>
    expect(relationLabel("spinoff")).toBe("Spinoff"));
  it("returns Same Series for series", () =>
    expect(relationLabel("series")).toBe("Same Series"));
  it("returns Adaptation for adaptation", () =>
    expect(relationLabel("adaptation")).toBe("Adaptation"));
  it("returns Remake / Remaster for remake", () =>
    expect(relationLabel("remake")).toBe("Remake / Remaster"));
  it("returns Related for related", () =>
...
describe("lookupMetadata — book (OpenLibrary)", () => {
  ...
  it("handles missing author_name and first_publish_year gracefully", async () => {
    const net = mockNet({
      docs: [{ key: "/works/OL789W", title: "Unknown Book" }],
    });
  ...
  it("passes the encoded title to OpenLibrary", async () => {
    ...
    const url = (net.fetch as ReturnType<typeof vi.fn>).mock
      .calls[0][0] as string;
...
describe("lookupMetadata — game (RAWG)", () => {
  ...
  it("includes the api key and encoded query in the RAWG URL", async () => {
    const net = mockNet({ results: [] });
    await lookupMetadata(net as never, "game", "Dark Souls", {
      rawgKey: "abc123",
    });
    const url = (net.fetch as ReturnType<typeof vi.fn>).mock
      .calls[0][0] as string;
  ...
  it("handles null released date gracefully", async () => {
    const net = mockNet({
      results: [{ id: 99, name: "Unreleased", released: null }],
    });
    const [result] = await lookupMetadata(net as never, "game", "Unreleased", {
      rawgKey: "k",
    });
  ...
  it("caps results at 5", async () => {
    ...
    const results = await lookupMetadata(net as never, "game", "game", {
      rawgKey: "k",
    });
    expect(results).toHaveLength(5);
  });
...
describe("lookupMetadata — movie / tv / anime (TMDB)", () => {
  ...
  it("uses name field for TV shows returned by TMDB", async () => {
    ...
    const [result] = await lookupMetadata(net as never, "tv", "Breaking Bad", {
      tmdbKey: "k",
    });
  ...
  it("includes the api key and encoded query in the TMDB URL", async () => {
    const net = mockNet({ results: [] });
    await lookupMetadata(net as never, "movie", "The Matrix", {
      tmdbKey: "tmdb-key",
    });
    const url = (net.fetch as ReturnType<typeof vi.fn>).mock
      .calls[0][0] as string;
  ...
  it("returns empty array when network response is not ok", async () => {
    const net = mockNet({}, false);
    const results = await lookupMetadata(net as never, "movie", "test", {
      tmdbKey: "k",
    });
    expect(results).toEqual([]);
  });
