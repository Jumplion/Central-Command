...
function MediaTracker({ api, settings, setTitle }: WidgetProps) {
  ...
  const [lookupResults, setLookupResults] = useState<ReturnType<typeof lookupMetadata> extends Promise<infer T> ? T : never>([]);
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <div style={{ flex: 1 }}>
                ...
                <select
                  ...
                  value={form.type}
                  onChange={e => {
                    setForm(f => ({ ...f, type: e.target.value as FormState['type'], external_id: '', external_source: '' }));
                    setLookupResults([]);
                  }}
                >
              ...
              <div style={{ flex: 1 }}>
                ...
                <select style={s.field} value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as FormState['status'] }))}>
