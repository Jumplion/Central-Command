...
function StatusTimeline({ history }: { history: HistoryEntry[] }) {
  ...
  return (
    <div
      style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid #1e293b" }}
    >
      {history.map((h, i) => {
  ...
  const candidates = allItems
    ...
    .filter(
      (item) =>
        ...
        (item.author_creator?.toLowerCase().includes(search.toLowerCase()) ??
          false),
    )
    .slice(0, 8);
...
export function ImportModal({
  ...
  const runImport = async () => {
    ...
    try {
      if (active === "steam") {
        if (!steamKey || !steamId) {
          setMessage(
            "Set Steam API Key and Steam ID in widget settings first.",
          );
          return;
        }
        ...
        if (games.length === 0) {
          setMessage("No games found in that library.");
          return;
        }
        ...
        const batch = games.map((g) => ({
          ...
          params: [
            g.name,
            (g.playtime_2weeks ?? 0) > 0 ? "current" : "owned",
            String(g.appid),
            now,
            now,
          ],
      ...
      if (active === "letterboxd") {
        if (!letterboxdUser) {
          setMessage("Set your Letterboxd username in widget settings first.");
          return;
        }
        const res = await api.net.fetch(
          `https://letterboxd.com/${letterboxdUser}/rss/`,
        );
        ...
        if (items.length === 0) {
          setMessage("No diary entries found for that username.");
          return;
        }
        const now = new Date().toISOString();
        const batch = items.map((item) => {
          const filmTitle =
            ...
            "";
          const filmId =
            item.getElementsByTagNameNS("*", "filmId")[0]?.textContent ??
            item.querySelector("guid")?.textContent ??
            "";
          const rawRating = item.getElementsByTagNameNS("*", "memberRating")[0]
            ?.textContent;
      ...
      if (active === "hardcover") {
        if (!hardcoverKey) {
          setMessage("Set your Hardcover API key in widget settings first.");
          return;
        }
        const res = await api.net.fetch(
        ...
        const statusMap: Record<number, MediaStatus> = {
          1: "want",
          2: "current",
          3: "completed",
          4: "dropped",
        ...
        if (books.length === 0) {
          setMessage("No books found in your Hardcover shelves.");
          return;
        }
        const now = new Date().toISOString();
  ...
  const descriptions: Record<ImportSection, string> = {
    steam:
      "Import your Steam game library. Requires a Steam Web API Key and your 64-bit Steam ID (set in widget settings).",
    letterboxd:
      "Import your Letterboxd diary as completed movies. Requires your public Letterboxd username (set in widget settings).",
    hardcover:
      "Import books from your Hardcover shelves. Requires a Hardcover API key (set in widget settings). Hardcover is a free Goodreads alternative.",
  };

  return (
    <div style={s.overlay} onClick={onClose}>
      <div
        style={{ ...s.modal, width: 380 }}
        onClick={(e) => e.stopPropagation()}
      >
        <h3
          style={{
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
        <span
          style={{ fontSize: 18, lineHeight: 1, marginTop: 3, flexShrink: 0 }}
        >
          {typeEmoji(item.type)}
        </span>
        ...
        <div style={{ marginTop: 8 }}>
          ...
          >
            ...
            >
              ...
              <button
                style={{ ...s.iconBtn, fontSize: 11, opacity: 0.8 }}
                onClick={onAddLink}
              >
                + Link
              </button>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
              ...
              {links.map((link) => {
                const otherId =
                  link.item_id === item.id ? link.linked_item_id : link.item_id;
