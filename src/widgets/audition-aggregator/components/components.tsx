  ...
  return (
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        ...
        <input
          style={inp}
          placeholder="Casting studio / director"
          value={form.casting_studio}
          onChange={set("casting_studio")}
        />
        <input
          style={inp}
          placeholder="Location (city, remote, self-tape…)"
          value={form.location}
          onChange={set("location")}
        />
        <input
          style={inp}
          placeholder="Pay rate (e.g. SAG scale, $250/day)"
          value={form.pay_rate}
...
export function WeeklyChart({ auditions }: { auditions: Audition[] }) {
  const data = useMemo(() => {
    ...
    return weeks.map(({ label, start }) => {
      ...
      STATUSES.forEach((s) => {
        entry[s] = weekAuds.filter((a) => a.status === s).length;
      });
      return entry;
    });
  }, [auditions]);
...
export function DeadlineCell({ date }: { date: string }) {
  ...
  const color =
    days < 0
      ? STATUS_COLOR.Passed
      : days <= 3
        ? STATUS_COLOR.Callback
        : undefined;
  return (
    <td
      style={{ padding: "5px 6px", color, fontWeight: color ? 600 : undefined }}
    >
      {date}
    </td>
  );
