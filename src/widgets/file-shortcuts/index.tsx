...
function FileShortcuts({ api }: WidgetProps) {
  ...
  const browse = async (kind: "file" | "dir") => {
    ...
    if (paths)
      await persist([...shortcuts, ...paths.map((p) => makeShortcut(p, kind))]);
  };
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
        <button
          style={buttonDefault}
          onClick={() => browse("file").catch(console.error)}
        >
          + File
        </button>
        <button
          style={buttonDefault}
          onClick={() => browse("dir").catch(console.error)}
        >
          + Folder
        </button>
      </div>
...
const widget: Widget = {
  manifest: {
    ...
    name: "File Shortcuts",
    description:
      "Quick-launch shortcuts to files and folders. Drag and drop to add.",
    version: "0.1.0",
    icon: "📂",
    defaultSize: { w: 4, h: 6 },
    minSize: { w: 3, h: 4 },
