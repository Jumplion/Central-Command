...
function exportCsvData(rows: UsageRow[]): void {
  ...
  const body = rows.map((r) => [
    ...
    r.mount_count > 0
      ? (r.total_duration_ms / r.mount_count / 1000).toFixed(1)
      : "0",
...
function WidgetAnalytics({ api, settings }: WidgetProps) {
  ...
  const dayCount = Math.max(
    1,
    Math.min(365, Number(settings?.chartDays ?? 14) || 14),
  );
  ...
  const loadData = useCallback(async () => {
    ...
    const [top, daily] = await Promise.all([
      ...
      api.sql.all<DailyRow>(
        ...namedSql(SELECT_DAILY_SERIES, { since_day: sinceDay }),
      ),
    ]);
    setTopWidgets(top);
  ...
  const handleExportCsv = useCallback(async () => {
    const allWidgets = await api.sql.all<UsageRow>(SELECT_ALL_WIDGETS);
    exportCsvData(allWidgets);
  }, [api]);
              ...
              <tbody>
                {topWidgets.map((r) => (
                  ...
                  >
                    ...
                    >
                      {r.last_accessed_at > 0
                        ? fmtRelative(r.last_accessed_at)
                        : "—"}
                    </td>
                  </tr>
                ))}
