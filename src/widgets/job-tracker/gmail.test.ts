...
import { parseJobStatus, parseCompany, parseRole, parseReqNumber, buildSuggestion, fetchJobEmails } from './gmail';
...
describe('job-tracker gmail parsers', () => {
  it('detects Offer before other status hints', () => {
    expect(parseJobStatus('We are excited to offer you a role', 'Interview scheduled')).toBe('Offer');
  });

  it('detects Rejected phrases', () => {
    expect(parseJobStatus('Thank you', 'Unfortunately we will not be moving forward')).toBe('Rejected');
  });

  it('detects Onsite interview language', () => {
    expect(parseJobStatus('Interview Invitation', 'Your onsite interview is scheduled')).toBe('Onsite');
  });

  it('detects Phone interview language', () => {
    expect(parseJobStatus('Next steps', 'Please schedule a phone screen')).toBe('Phone');
  });

  it('defaults to Applied when no other status matches', () => {
    expect(parseJobStatus('Hello', 'Thanks for your application')).toBe('Applied');
  });
  ...
  it('parses role from explicit title patterns', () => {
    expect(parseRole('Position: Senior Software Engineer', '')).toBe('Senior Software Engineer');
  });

  it('parses role from "for the ... position" patterns', () => {
    expect(parseRole('Hello', 'We would like to invite you for the Backend Engineer position')).toBe('Backend Engineer');
  });
...
describe('fetchJobEmails', () => {
  ...
  it('skips already known gmail ids and does not fetch details again', async () => {
    const api = makeMockApi() as any;
    api.net.fetch.mockResolvedValueOnce({ ok: true, body: JSON.stringify({ messages: [{ id: 'existing', threadId: 't1' }] }) });
  ...
  it('throws when the Gmail list endpoint returns a failure', async () => {
    ...
    await expect(fetchJobEmails(api, 'token-abc')).rejects.toThrow('Gmail list failed: 401');
  });

  it('continues when an individual message fetch fails', async () => {
    const api = makeMockApi() as any;
    api.net.fetch
      .mockResolvedValueOnce({ ok: true, body: JSON.stringify({ messages: [{ id: 'msg-1', threadId: 'thread-1' }] }) })
