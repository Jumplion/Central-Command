...
describe('isValidWidgetId', () => {
  it('accepts a single lowercase letter', () => expect(isValidWidgetId('a')).toBe(true));
  it('accepts alphanumeric ids', () => expect(isValidWidgetId('widget123')).toBe(true));
  it('accepts hyphens within the id', () => expect(isValidWidgetId('job-aggregator')).toBe(true));
  ...
  it('rejects ids that start with a hyphen', () => expect(isValidWidgetId('-widget')).toBe(false));
  it('rejects ids with uppercase letters', () => expect(isValidWidgetId('MyWidget')).toBe(false));
  it('rejects ids exceeding 64 characters', () => {
    expect(isValidWidgetId('a' + 'b'.repeat(64))).toBe(false);
  });
  it('rejects ids with underscores', () => expect(isValidWidgetId('my_widget')).toBe(false));
  it('rejects ids with dots', () => expect(isValidWidgetId('widget.one')).toBe(false));
  it('rejects ids with spaces', () => expect(isValidWidgetId('my widget')).toBe(false));
...
describe('assertValidWidgetId', () => {
  ...
  it('throws for a path-traversal id', () => {
    expect(() => assertValidWidgetId('../evil')).toThrow('Invalid widget id: ../evil');
  });
