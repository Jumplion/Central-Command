...
describe("GOOGLE_SERVICES", () => {
  it("defines API base URLs for all built-in services", () => {
    expect(GOOGLE_SERVICES.gmail.apiBaseUrl).toBe(
      "https://gmail.googleapis.com/gmail/v1/",
    );
    expect(GOOGLE_SERVICES.calendar.apiBaseUrl).toBe(
      "https://www.googleapis.com/calendar/v3/",
    );
    expect(GOOGLE_SERVICES.drive.apiBaseUrl).toBe(
      "https://www.googleapis.com/drive/v3/",
    );
    expect(GOOGLE_SERVICES.contacts.apiBaseUrl).toBe(
      "https://people.googleapis.com/v1/",
    );
    expect(GOOGLE_SERVICES.notes.apiBaseUrl).toBe(
      "https://keep.googleapis.com/v1/",
    );
  });
  ...
  it("keeps every service definition structurally complete", () => {
    for (const [serviceId, definition] of Object.entries(GOOGLE_SERVICES)) {
      ...
      expect(
        definition.defaultScopes.every((scope) => scope.startsWith("https://")),
      ).toBe(true);
    }
  });
  ...
  it("uses expected default scopes for built-in services", () => {
    ...
    expect(GOOGLE_SERVICES.calendar.defaultScopes).toEqual([
      "https://www.googleapis.com/auth/calendar.readonly",
    ]);
    expect(GOOGLE_SERVICES.drive.defaultScopes).toEqual([
...
describe("stored Google creds helpers", () => {
  ...
  it("returns null for invalid stored Google credentials", () => {
    ...
    expect(
      parseStoredGoogleCreds(JSON.stringify({ clientId: "id" })),
    ).toBeNull();
  });
