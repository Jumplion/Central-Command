...
export function resolveGoogleScopes(options: GoogleConnectOptions): string[] {
  ...
  if (options.service)
    return [...GOOGLE_SERVICES[options.service].defaultScopes];
...
export function parseStoredGoogleCreds(
  raw: string | null,
): StoredGoogleCreds | null {
  if (!raw) return null;
  try {
...
export function getGoogleReconnectOptions(
  raw: string | null,
  service?: GoogleServiceId,
): GoogleConnectOptions | null {
