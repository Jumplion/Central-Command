...
vi.mock("electron", () => ({
  ...
  dialog: {
    showOpenDialog: vi
      .fn()
      .mockResolvedValue({ canceled: false, filePaths: ["/tmp/selected"] }),
  },
  net: {
...
describe("KV handlers", () => {
  ...
  it("throws on kv.keysWithPrefix when prefix is not a string", () => {
    expect(() => call(IPC.KV_KEYS_PREFIX, "w", 99)).toThrow(
      "invalid arguments",
    );
  });
});
...
describe("SQL handlers", () => {
  it("throws when widgetId is not a string", () => {
    expect(() => call(IPC.SQL_RUN, 42, "SELECT 1", [])).toThrow(
      "invalid arguments",
    );
    expect(() => call(IPC.SQL_ALL, null, "SELECT 1", [])).toThrow(
      "invalid arguments",
    );
    expect(() => call(IPC.SQL_GET, {}, "SELECT 1", [])).toThrow(
      "invalid arguments",
    );
  });
  ...
  it("throws when params is not an array", () => {
    expect(() => call(IPC.SQL_ALL, "w", "SELECT 1", "not-an-array")).toThrow(
      "invalid arguments",
    );
    expect(() => call(IPC.SQL_GET, "w", "SELECT 1", {})).toThrow(
      "invalid arguments",
    );
  });

  it("throws on sql.exec when widgetId is not a string", () => {
    expect(() => call(IPC.SQL_EXEC, 42, "SELECT 1")).toThrow(
      "invalid arguments",
    );
  });

  it("throws on sql.runBatch when items is not an array", () => {
    expect(() => call(IPC.SQL_RUN_BATCH, "w", "not-an-array")).toThrow(
      "invalid arguments",
    );
  });
  ...
  it("delegates sql.run to storage.sqlite.run", () => {
    ...
    expect(mockStorage.sqlite.run).toHaveBeenCalledWith(
      "w",
...
describe("SHELL_OPEN_EXTERNAL", () => {
  it("throws for ftp:// URLs", () => {
    expect(() => call(IPC.SHELL_OPEN_EXTERNAL, "ftp://example.com")).toThrow(
  ...
  it("throws for javascript: URLs", () => {
    expect(() =>
      call(IPC.SHELL_OPEN_EXTERNAL, "javascript:alert(1)"),
    ).toThrow();
  });
  ...
  it("accepts https:// URLs without throwing", () => {
    expect(() =>
      call(IPC.SHELL_OPEN_EXTERNAL, "https://example.com"),
    ).not.toThrow();
  });

  it("accepts http:// URLs without throwing", () => {
    expect(() =>
      call(IPC.SHELL_OPEN_EXTERNAL, "http://example.com"),
    ).not.toThrow();
  });

  it("accepts mailto: URLs without throwing", () => {
    expect(() =>
      call(IPC.SHELL_OPEN_EXTERNAL, "mailto:user@example.com"),
    ).not.toThrow();
  });
});
...
describe("SHELL_SHOW_IN_FOLDER", () => {
  it("throws when path is not a string", () => {
    expect(() => call(IPC.SHELL_SHOW_IN_FOLDER, null)).toThrow(
      "invalid argument",
    );
  });
});
...
describe("NET_FETCH", () => {
  it("rejects non-http(s) URLs", async () => {
    await expect(call(IPC.NET_FETCH, "ftp://bad.com", {})).rejects.toThrow(
      "url must be http(s)",
    );
  });

  it("rejects non-string url", async () => {
    await expect(call(IPC.NET_FETCH, null, {})).rejects.toThrow(
      "url must be http(s)",
    );
  });
  ...
  it("rejects javascript: URLs", async () => {
    await expect(
      call(IPC.NET_FETCH, "javascript:alert(1)", {}),
...
describe("Secrets handlers", () => {
  it("throws when widgetId is not a string", () => {
    ...
    expect(() => call(IPC.SECRETS_DEL, null, "key")).toThrow(
      "invalid arguments",
    );
  ...
  it("throws on secrets.set when value is not a string", () => {
    expect(() => call(IPC.SECRETS_SET, "w", "key", 123)).toThrow(
      "invalid arguments",
    );
    expect(() => call(IPC.SECRETS_SET, "w", "key", null)).toThrow(
      "invalid arguments",
    );
  });
...
describe("GOOGLE_CONNECT", () => {
  it("throws when widgetId is not a string", () => {
    expect(() =>
      call(IPC.GOOGLE_CONNECT, 42, { clientId: "id", clientSecret: "sec" }),
    ).toThrow("invalid arguments");
  });

  it("throws when options is null or not an object", () => {
    expect(() => call(IPC.GOOGLE_CONNECT, "w", null)).toThrow(
      "invalid arguments",
    );
    expect(() => call(IPC.GOOGLE_CONNECT, "w", "string")).toThrow(
      "invalid arguments",
    );
  });
  ...
  it("throws when clientId is missing", () => {
    expect(() =>
      call(IPC.GOOGLE_CONNECT, "w", { clientSecret: "sec" }),
    ).toThrow("invalid options");
  });

  it("throws when clientSecret is missing", () => {
    expect(() => call(IPC.GOOGLE_CONNECT, "w", { clientId: "id" })).toThrow(
      "invalid options",
    );
  });
...
describe("Google service handlers (getToken / disconnect / isConnected)", () => {
  it("throws when widgetId is not a string", () => {
    ...
    expect(() => call(IPC.GOOGLE_DISCONNECT, null)).toThrow(
      "invalid arguments",
    );
    expect(() => call(IPC.GOOGLE_IS_CONNECTED, {})).toThrow(
      "invalid arguments",
    );
  });

  it("throws when service is present but not a known id", () => {
    expect(() => call(IPC.GOOGLE_GET_TOKEN, "w", "not-a-service")).toThrow(
      "invalid service",
    );
    expect(() => call(IPC.GOOGLE_DISCONNECT, "w", "bad-service")).toThrow(
      "invalid service",
    );
  });
...
describe("KV delegation", () => {
  it("delegates kv.set to storage.json.set", async () => {
    ...
    expect(mockStorage.json.set).toHaveBeenCalledWith("w", "key", {
      nested: true,
    });
  });
...
describe("SQL delegation", () => {
  it("delegates sql.all to storage.sqlite.all", () => {
    ...
    expect(mockStorage.sqlite.all).toHaveBeenCalledWith(
      "w",
      "SELECT * FROM t",
      [],
    );
  ...
  it("delegates sql.get to storage.sqlite.get", () => {
    ...
    expect(result).toEqual({ id: 1 });
    expect(mockStorage.sqlite.get).toHaveBeenCalledWith(
      "w",
  ...
  it("delegates sql.exec to storage.sqlite.exec", () => {
    ...
    expect(mockStorage.sqlite.exec).toHaveBeenCalledWith(
      "w",
  ...
  it("delegates sql.runBatch to storage.sqlite.runBatch", () => {
    ...
    mockStorage.sqlite.runBatch.mockReturnValue([
...
describe("DIALOG_OPEN_PATH", () => {
  it("returns selected file paths when not canceled", async () => {
    ...
    expect(await call(IPC.DIALOG_OPEN_PATH, {})).toEqual([
      "/home/user/doc.txt",
    ]);
  });
