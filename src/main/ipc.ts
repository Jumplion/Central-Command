...
type ArgDescriptor =
  | ArgValidator
  | { validator: ArgValidator; optional: boolean };
...
export function registerIpc(
  ...
  registerHandler(IPC.STATE_SAVE, [isObject], (state: AppState) =>
  ...
  registerHandler(
    IPC.KV_GET,
    [isString, isString],
    (widgetId: string, key: string) => storage.json.get(widgetId, key),
  );
  registerHandler(
    IPC.KV_SET,
    [isString, isString],
    (widgetId: string, key: string, value: unknown) =>
      storage.json.set(widgetId, key, value),
  );
  registerHandler(
    IPC.KV_DEL,
    [isString, isString],
    (widgetId: string, key: string) => storage.json.del(widgetId, key),
  );
  registerHandler(IPC.KV_KEYS, [isString], (widgetId: string) =>
    storage.json.keys(widgetId),
  );
  registerHandler(
    IPC.KV_KEYS_PREFIX,
    [isString, isString],
    (widgetId: string, prefix: string) =>
  ...
  registerHandler(
    IPC.SQL_RUN,
    [isString, isString, isParams],
    (widgetId: string, sql: string, params: unknown[]) =>
  ...
  registerHandler(
    IPC.SQL_EXEC,
    [isString, isString],
    (widgetId: string, sql: string) => storage.sqlite.exec(widgetId, sql),
  );
  registerHandler(
    IPC.SQL_RUN_BATCH,
  ...
  registerHandler(IPC.SHELL_OPEN_EXTERNAL, [isString], (url: string) => {
    if (!/^(https?|mailto):/.test(url))
      throw new Error("openExternal: url must be http(s) or mailto");
    return openExternal(url);
  });

  registerHandler(IPC.SHELL_OPEN_PATH, [isString], (path: string) =>
    shell.openPath(path),
  );
  registerHandler(IPC.SHELL_SHOW_IN_FOLDER, [isString], (path: string) =>
    shell.showItemInFolder(path),
  );

  registerHandler(
    IPC.DIALOG_OPEN_PATH,
    [{ validator: () => true, optional: true }],
  ...
  registerHandler(
    IPC.NET_FETCH,
    [() => true, { validator: isObject, optional: true }],
    async (url: unknown, init: unknown) => {
      if (!isString(url) || !/^https?:\/\//.test(url))
        throw new Error("net.fetch: url must be http(s)");
      ...
      if (isObject(init)) {
        ...
        if (i.headers && typeof i.headers === "object")
          options.headers = i.headers as Record<string, string>;
  ...
  registerHandler(
    IPC.SECRETS_GET,
    [isString, isString],
    (widgetId: string, key: string) => secrets.get(widgetId, key),
  );
  registerHandler(
    IPC.SECRETS_DEL,
    [isString, isString],
    (widgetId: string, key: string) => secrets.del(widgetId, key),
  );
  registerHandler(
    IPC.SECRETS_HAS,
    [isString, isString],
    (widgetId: string, key: string) => secrets.has(widgetId, key),
  );
  registerHandler(
    IPC.SECRETS_SET,
  ...
  registerHandler(
    IPC.GOOGLE_CONNECT,
    [isString, isObject],
    (widgetId: string, options: unknown) => {
      if (!isObject(options))
      ...
      if (o.scopes !== undefined && !Array.isArray(o.scopes))
        throw new Error("google.connect: invalid scopes");
      requireOptionalService("google.connect", o.service);
      return oauth.connect(
        widgetId,
        options as unknown as GoogleConnectOptions,
      );
    },
  );
  ...
  const googleServiceHandler = (
    ...
    registerHandler(
      channel,
      [isString, { validator: () => true, optional: true }],
      (widgetId: string, service?: unknown) => {
